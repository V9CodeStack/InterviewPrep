...
`,
  ...
  {
    id: 36,
    question: `36. what is difference between package.josn and package lock.json in ReactJS ?`,
    answer: [
      `package.json is a configuration file that contains metadata about your project and its dependencies`,
      `It includes information such as the project's name, version, description, author, and other settings`,
      `It also lists the dependencies your project relies on, both direct dependencies (those you explicitly install) and their versions.`,
      `package-lock.json is an automatically generated file that provides a detailed snapshot of the exact versions of dependencies and their dependencies (sub-dependencies).`,
      `It ensures that all developers working on the project use the exact same versions of dependencies, preventing subtle differences in development environments.`,
      `The purpose of package-lock.json is to maintain consistency across installations and deployments. It helps prevent situations where one developer might install slightly different versions of packages, which could lead to compatibility issues or unexpected behavior.`,
    ],
    code: ``,
    video: ``,
  },
  {
    id: 37,
    question: `37. what is difference between single page application and multipage application ?`,
    answer: [
      `Single Page Application (SPA):`,
      `Loads as a single HTML page initially.`,
      `Dynamically updates content without full page reloads.`,
      `Uses client-side rendering (CSR) for interactivity.`,
      `Better for responsive and interactive user experiences.`,
      `Multi-Page Application (MPA):`,
      `Each view is a separate HTML page.`,
      `Navigates between pages with full reloads.`,
      `Uses server-side rendering (SSR) for faster initial load times.`,
      `Better for SEO and initial load performance.`,
      `Might have slower navigation and responsiveness.`,
    ],
    code: ``,
    video: ``,
  },
  {
    id: 38,
    question: `38. what is Webpack ?`,
    answer: [
      `webpack is used to stitch together a group of modules into a  single filr (or group of files), 
  ...
  {
    id: 39,
    question: `39. How Eventloop work in node js ?`,
    answer: [
      ` Node.js event loop efficiently manages asynchronous operations by continuously checking for pending tasks in different phases, executing their associated callbacks, and allowing the application to handle multiple tasks concurrently without blocking the main thread.`,
    ],
    code: ``,
    video: ``,
  },
  {
    id: 40,
    question: `40. reactjs event loop and nodejs event loop is same ?`,
    answer: [
      `Node.js Event Loop:`,
      `In Node.js, the event loop is a core part of the runtime environment and is responsible for managing asynchronous operations like I/O, timers, and callbacks.`,
      `Node.js operates on the server side and handles tasks such as file I/O, network requests, and database operations.`,
      `The event loop in Node.js ensures non-blocking behavior and efficient handling of concurrent operations.`,
      `React Event Loop:`,
      `React's event loop is primarily focused on managing user interface updates and interactions in the browser.`,
      `It's based on the browser's native event loop and is responsible for handling events like user input, rendering updates, and interactions with the DOM.`,
      `React's event loop facilitates the virtual DOM diffing process to efficiently update the actual DOM based on changes in application state.`,
    ],
    code: ``,
    video: ``,
  },
  {
    id: 41,
    question: `41. what is cors issue and how to resolve it ?`,
    answer: [
      `CORS (Cross-Origin Resource Sharing) is a security feature implemented by web browsers to prevent unauthorized access to resources (e.g., APIs) from different origins (domains).`,
      `Browsers enforce the "same-origin policy," allowing requests only from the same origin (domain, protocol, and port) as the web page making the request.`,
      `If a web page tries to access resources from a different origin, the browser blocks the request due to CORS, causing a "CORS issue" or "CORS error."`,
      `Resolution:`,
      `On the server providing the resource, configure CORS headers to explicitly allow requests from specific origins.`,
      `Use the Access-Control-Allow-Origin header to specify which domains are allowed to access the resource.`,
    ],
    code: ``,
    video: ``,
  },
